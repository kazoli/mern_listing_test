...
import { tsValidation } from "./vaildationTypes";

export const processName = (value: string, params: tsValidation["name"]) => {
...
export const processTags = (values: string[], params: tsValidation["tag"]) => {
  ...
  values.map((tag) => {
    tag = tag.trim();
    if (tag.length < params.minLength || tag.length > params.maxLength)
      tagError = true;
    return tag;
  });
  if (tagError) {
